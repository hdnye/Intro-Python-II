...
class Store:
    def __init__(self, name, departments):
        ...
        self._departments: List[Department] = departments
    def __str__(self):
        return f"{self.name}: {[department.name for department in self._departments]}"

    def print_welcome(self):
        print(f"Welcome to {self.name}!")
        print("Browse our departments:")
        for i, department in enumerate(self._departments):
            print(f"{i+1}. {department.name}")

    def get_department_by_id(self, department_id):
        return self._departments[department_id - 1]
...
while True:
    ...
    # that is more encapsulated? This is showing that depts is being stored as a list tot he user
        department = store.get_department_by_id(int(user_input))
        print(f"You chose department {department}")
    # TODO if there are no prodcuts, print a nice stmt
    except IndexError as e:
        print('Please enter a valid dept id') 
